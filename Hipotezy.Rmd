...
mindane <- c("Min", mindane)
maxdane <- c("Max", maxdane)
mediadane <- c("Mediana", mediadane)
normalnosc <- c("Czy rozkład jest normalny", normalnosc)

labelki <- c("Cecha", "wiek", "zly_humor", "swietnie",
         ...
         "znaczenie_kontaktow", "suma_samop1sr", "suma_samop2sr", "ocena_samop_ogolna")
```

```{r tabelka}
tabela_cala <- cbind(labelki,mindane, mediadane, maxdane, normalnosc)
tabela_cala <- as.tabular(tabela_cala)
```

```{r tabelka cała kable, results='asis'}
toKable(tabela_cala) %>%
