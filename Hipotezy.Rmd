...
P-value dla testu `rho-Spearmana` wynosi `r sprintf("%.3f", r3)`, natomiast dla testu `tau-Kendalla` wynosi `r sprintf("%.3f", r4)`. Nie ma podstaw do odrzucenia $H_0$. Nie istnieje istotna zależność pomiędzy dietą a poczuciem radości.
...
P-value dla testu `rho-Spearmana` wynosi `r sprintf("%.3f", r5)`, natomiast dla testu `tau-Kendalla` wynosi `r sprintf("%.3f", r6)`. Należy odrzucić hipotezę $H_0$, na rzecz hipotezy alternatywnej $H_1$. Istnieje zależność pomiędzy liczbą osób, z którymi utrzymuje się regularny kontakt a odczuwaniem niepokoju.
...
P-value dla testu `rho-Spearmana` wynosi `r sprintf("%.3f", r13)`, natomiast dla testu `tau-Kendalla` wynosi `r sprintf("%.3f", r14)`. Należy odrzucić hipotezę $H_0$, na rzecz hipotezy alternatywnej $H_1$. Istnieje zależność pomiędzy jakością posiłków a dobrym humorem.
...
P-value z testu Shapio-Wilka dla cechy Ogólna ocena samopoczucia wynosi `r sprintf("%.3f", shapiro)`, zatem ta cecha nie ma rozkładu normalnego.  Stosujemy test Manna - Whitney'a - Wilcoxona.
...
P-value z testu Manna - Whitney'a - Wilcoxona wynosi `r sprintf("%.3f", pval)`. Istnieje zależność pomiędzy płcią a ogólną oceną samopoczucia.
...
Test F wykazuje, że wariancje Ogólnej oceny samopoczucia są równe w grupach kobiet i mężczyzn (p-value `r sprintf("%.3f", pval)`), zatem stosujemy test T dla średnich.
