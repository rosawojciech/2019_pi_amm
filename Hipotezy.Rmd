...
Płeć a ogólna ocena samopoczucia




```{r}
shapiro.test(dane$ocena_samop_ogolna) #nie ma rozkładu normalnego
...
```{r}
wilcox.test(ocena_samop_ogolna~plec, data = dane)
```
Istnieje zależność pomiędzy płcią a ogólną oceną samopoczucia.

## Hipoteza IX

Wiek a ogólna ocena samopoczucia
...
Na podstawie powyższych testów mamy, że model spełnia założenia Gaussa - Markova, a jedynym problemem jest brak normalności rozkładu reszt (chociaż na wykresach diagnostycznych wygląda trochę na heterogeniczność wariancji).
