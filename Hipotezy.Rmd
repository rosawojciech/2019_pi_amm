...
```{r hipoteza 9  testy}
r11 <- cor.test(x = dane$wiek,
               y = dane$ocena_samop_ogolna,
               method = "spearman")$p.value
r12 <- cor.test(x = dane$wiek,
               y = dane$ocena_samop_ogolna,
               method = "kendall")$p.value
```
...
Z testów Spearmana oraz Kendalla otrzymujemy p-value około 0.01, co pozwala odrzucić hipotezę zerową o braku zależności. Współczynniki korelacji wynoszą odpowiednio 0.163 dla Spearmana i 0.116 dla Kendalla. Im więcej mamy aktywności, tym lepsze nasze samopoczucie :)
...
![width=400px](infografika.png){Wpływ badanych czynników na samopoczucie. Pod czynnikiem podany współczynnik korelacji (na tle zielonym dodatnia, na czerwonym ujemna)}
